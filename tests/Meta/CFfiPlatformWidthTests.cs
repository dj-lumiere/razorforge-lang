...
public sealed class CFfiPlatformWidthTests
{
    ...
    public void CFfiScalarWrappers_TakePlatformWidth()
    {
        ...
        Assert.True(condition: exitCode == 0 && !timedOut,
            userMessage:
            $"buildandrun failed (exit={exitCode}, timedOut={timedOut}).\n--- stdout ---\n{stdout}\n--- stderr ---\n{stderr}");

        string actual = KeepPrefixed(stdout: stdout,
            "CLong=",
            "CULong=",
            "CWChar=");
        Assert.True(condition: actual == expected,
            userMessage:
    ...
    public static TheoryData<string, string, string, string> BoundaryCases()
    {
        return new TheoryData<string, string, string, string>
        {
            // ctorType, value literal, widen routine, expected decimal on Unix
            {
                "CLong", "2147483648s64", "S64", "2147483648"
            }, // 2^31 — 1 past Windows 32-bit signed long
            {
                "CULong", "4294967296u64", "U64", "4294967296"
            }, // 2^32 — 1 past Windows 32-bit unsigned long
            { "CWChar", "65536u32", "U32", "65536" } // 2^16 — 1 past Windows 16-bit wchar_t
        };
    }

    [Theory]
    [MemberData(memberName: nameof(BoundaryCases))]
    public void CFfiNarrowing_AtBoundary_DivergesByPlatform(string ctorType, string valueLiteral,
        string widen, string expectedUnixDecimal)
    {
        ...
        else
        {
            ...
            Assert.Equal(expected: $"v={expectedUnixDecimal}",
                actual: KeepPrefixed(stdout: stdout, "v="));
        }
    }
    ...
    public void CWStr_SupplementaryCodeUnit_DivergesByPlatform()
    {
        ...
        (int exitCode, string stdout, string stderr, bool timedOut) = RunProgram(source: source);
        Assert.False(condition: timedOut,
            userMessage: "buildandrun timed out for the CWStr supplementary probe.");
        ...
        if (IsWindows)
        {
            ...
            Assert.True(
                condition: (stdout + "\n" + stderr).Contains(value: "IntegerOverflowError",
                    comparisonType: StringComparison.Ordinal),
                userMessage:
        ...
        else
        {
            ...
            Assert.Equal(expected: "cp=65536 count=1",
                actual: KeepPrefixed(stdout: stdout, "cp="));
        }
    }

    private static string PlatformName => IsWindows
        ? "Windows (LLP64)"
        : "Unix (LP64)";

    /// <summary>Writes <paramref name="source"/> to an isolated temp dir and runs it through buildandrun.</summary>
    private static (int ExitCode, string Stdout, string Stderr, bool TimedOut) RunProgram(
        string source)
    {
        ...
        string tempDir = Path.Combine(path1: Path.GetTempPath(),
            path2: "rf-cffi-" + Guid.NewGuid()
                                    .ToString(format: "N"));
        Directory.CreateDirectory(path: tempDir);
        string probe = Path.Combine(path1: tempDir, path2: "probe.rf");
        try
        {
            File.WriteAllText(path: probe, contents: source);
            return RunBuildAndRun(rfPath: probe);
        }
        finally
        {
            try { Directory.Delete(path: tempDir, recursive: true); }
            catch
            {
                /* best effort */
            }
    ...
    private static (int ExitCode, string Stdout, string Stderr, bool TimedOut) RunBuildAndRun(
        string rfPath)
    {
    ...
    private static string KeepPrefixed(string stdout, params string[] prefixes)
    {
        ...
        foreach (string raw in stdout.Replace(oldValue: "\r\n", newValue: "\n")
