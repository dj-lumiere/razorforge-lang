...
public sealed class BaseEmissionTests
{
    ...
    /// </summary>
    [Fact(Skip = "WIP resident-JIT base define-completeness oracle. Base BUILDS at gap=140/defined=12249 (was 275, was 695). ISOLATED-BUILD primitive (GenericClosurePass.RunIsolatedTail, post-fixpoint, build-one-no-drain) closes: (1) entity self-free tail via call-driven closure, (2) the destroy tail by building destroy on EVERY registered concrete+wrapper instance (bounded, self-contained: each field.destroy callee is itself registered; skip unfolded-comptime Array[U8,${...}] carrier instances). Remaining 140 = represent(98) + cyclic_trace(38) + serialize(1): represent is a force-seeded display closure that does NOT converge (adding it took gap 140->367 — its callees add net declares); needs a different (delta or codegen-materialize) approach. base+delta coverage proven by GenerateBase_And_Delta_CoverPrunedBuild_WithTinyDelta. See .claude-memory/base-completeness-const-generic-array-gap.md.")]
