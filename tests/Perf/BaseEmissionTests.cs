...
public sealed class BaseEmissionTests
{
    ...
    /// </summary>
    [Fact(Skip = "WIP resident-JIT base define-completeness oracle. Base BUILDS at gap=275/defined=10710 (was 695/9450). The ISOLATED-BUILD primitive (GenericClosurePass.RunIsolatedTail, post-fixpoint, build-one-no-drain via ProcessResolvedMemberRoutineGenericRoutine) materializes the entity self-free tail (hijack/invalidate) by deterministic seed + a CALL-DRIVEN closure over the actual lifecycle call graph — no crash, no runaway, no over-build (never touches the uncalled Array[SerialValue,63].assign). Remaining 275 = codegen-injected field-walk destroy/represent + cyclic_trace (no AST call to discover; deterministic enumeration over-builds) and const-generic Array[X,N] lifecycle (resolved-routine builder does not substitute const-generic N -> Unknown identifier N). base+delta coverage proven by GenerateBase_And_Delta_CoverPrunedBuild_WithTinyDelta. See .claude-memory/base-completeness-const-generic-array-gap.md.")]
