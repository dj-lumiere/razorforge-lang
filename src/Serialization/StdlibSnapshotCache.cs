...
public static class StdlibSnapshotCache
{
        ...
        Action<string>? log = null)
    {
        lock (_memo)
        {
            if (_memo.TryGetValue(key: language,
                    value: out SemanticVerifier.CompiledStdlibState? memoized))
            {
                return memoized;
            }
        }

        SemanticVerifier.CompiledStdlibState result =
            LoadOrCaptureUncached(language: language, log: log);
        lock (_memo)
        {
            _memo[key: language] = result;
        }
        ...
        Action<string>? log)
    {
        ...
        if (TryLoadMonolithic(language: language, path: path, log: log) is { } cached)
        {
            return cached;
        }

        var swCap = System.Diagnostics.Stopwatch.StartNew();
        SemanticVerifier.CompiledStdlibState fresh =
            SemanticVerifier.CaptureCompiledStdlib(language: language);
        log?.Invoke(obj: $"captured {language} stdlib fresh ({swCap.ElapsedMilliseconds} ms)");
    ...
    private static SemanticVerifier.CompiledStdlibState? TryLoadMonolithic(Language language,
        string? path, Action<string>? log)
    {
        try
        {
            if (path != null && File.Exists(path: path))
            {
                ...
                using var ms = new MemoryStream(buffer: File.ReadAllBytes(path: path),
                    writable: false);
                SemanticVerifier.CompiledStdlibState state =
                    PbrfSerializer.Deserialize<SemanticVerifier.CompiledStdlibState>(stream: ms);
                log?.Invoke(
                    obj:
                    $"loaded {language} stdlib from {Path.GetFileName(path: path)} ({sw.ElapsedMilliseconds} ms)");
                return state;
            }
        }
        catch (Exception ex)
        ...
        Action<string>? log)
    {
        ...
        try
        {
            ...
            log?.Invoke(obj: $"wrote {Path.GetFileName(path: path)}");
        }
        catch (Exception ex)
        {
    ...
    private static SemanticVerifier.CompiledStdlibState? TryLoadModular(Language language,
        Action<string>? log)
    {
        try
        {
    ...
    private static string? ComputeHash(Language language)
    {
        ...
        if (string.IsNullOrEmpty(value: root) || !Directory.Exists(path: root))
        {
            return null;
        }
        ...
        sb.Append(value: 'v')
          .Append(value: PbrfSerializer.FormatVersion)
          .Append(value: ';');
        sb.Append(value: language)
          .Append(value: ';');
        ...
        try
        {
            ...
            {
                sb.Append(value: "asm|")
                  .Append(value: new FileInfo(fileName: asm).LastWriteTimeUtc.Ticks);
            }
        }
        catch
        {
        ...
        return Convert.ToHexString(inArray: digest)
                      .AsSpan(start: 0, length: 16)
                      .ToString()
                      .ToLowerInvariant();
    }
    ...
    private static void AppendSourceFileMetadata(StringBuilder sb, string root, Language language)
    {
        ...
        foreach ((string dir, string glob) in roots)
        {
                                                  ...
                                                  comparer: StringComparer.Ordinal))
            {
                ...
                sb.Append(value: file)
                  .Append(value: '|')
                  .Append(value: fi.Length)
                  .Append(value: '|')
                  .Append(value: fi.LastWriteTimeUtc.Ticks)
                  .Append(value: '\n');
            }
        }
    }
