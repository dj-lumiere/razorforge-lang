...
internal static class RoutineGenericParameters
{
    ...
    {
        if (routine.GenericConstraints is not { } constraints) return;
        foreach (GenericConstraintDeclaration constraint in constraints)
        {
            if (constraint.ConstraintType != ConstraintKind.AnyType) continue;
            routine.GenericParameters ??= [];
            if (!routine.GenericParameters.Contains(constraint.ParameterName))
                routine.GenericParameters.Add(constraint.ParameterName);
        }
    }
}
