...
public sealed partial class SemanticVerifier
{
    ...
    /// built via <see cref="Synthesis.ErrorHandlingVariantPass.GenerateVariantBody"/>; its broad-propagation
        ...
        while (_variantBodyGenQueue.Count > 0)
        {
            ...
            foreach (GeneratedVariant variant in variants)
            {
                ...
                if (_variantBodies.ContainsKey(key: key) || _restoredVariantKeys.Contains(item: key))
                    continue;
                _variantBodies[key: key] = Compiler.Synthesis.ErrorHandlingVariantPass.GenerateVariantBody(
