...
internal sealed class ExpressionLoweringPass(PostprocessingContext ctx)
{
    ...
    private (List<Statement> Hoisted, Expression Expr) LowerSetLiteral(SetLiteralExpression set)
    {
        ...
        foreach (Expression elem in set.Elements)
        {
            var (h, lowered) = LowerExpr(elem);
            hoisted.AddRange(h);
            hoisted.Add(MakeCollectionAddCall(colRef, setType, Declaration.RuntimeContract.Collection.Add, [lowered], loc));
        }
    ...
    private (List<Statement> Hoisted, Expression Expr) LowerDictLiteral(DictLiteralExpression dict)
    {
        ...
        foreach ((Expression key, Expression value) in dict.Pairs)
        {
            ...
            hoisted.Add(MakeCollectionAddCall(colRef, dictType, Declaration.RuntimeContract.Collection.Add, args, loc));
        }
    ...
    private static TypeInfo? UnwrapOwnershipWrapper(TypeInfo? type)
    {
        ...
        if (type is RecordTypeInfo { TypeArguments: { Count: 1 } recArgs } rec
            && (rec.GenericDefinition?.Name is Declaration.RuntimeContract.Owned or Declaration.RuntimeContract.Retained or Declaration.RuntimeContract.Tracked
                || GetCollectionBaseName(rec) is Declaration.RuntimeContract.Owned or Declaration.RuntimeContract.Retained or Declaration.RuntimeContract.Tracked))
        {
            return recArgs[0];
        }
        return null;
        ...
        IsPatternExpression ipe, Expression loweredExpr, TypeInfo? boolType, List<Statement> hoisted)
    {
        var presentAccess = new MemberExpression(
            Object: loweredExpr, MemberName: Declaration.RuntimeContract.Carrier.PresentField, Location: ipe.Location)
        {
            ResolvedType = boolType
        };
        if (ipe.IsNegated)
